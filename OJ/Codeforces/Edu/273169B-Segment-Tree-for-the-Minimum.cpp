...
void merge(int c, int l, int r)
{
    int left = 2 * c;
    int right = left + 1;
    segmentTree[c].val = min(segmentTree[left].val , segmentTree[right].val);
}
...
int query_min(int c, int l, int r, int i, int j)
{
    if (l > j || r < i)
        return INT64_MAX;
    else if (l >= i && r <= j)
    ...
    int mid = (l + r) / 2;
    return min(query_min(left, l, mid, i, j) , query_min(right, mid + 1, r, i, j));
}
int32_t main()
{

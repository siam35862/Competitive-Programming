...
// Lazy Max-Heaps for light frequencies [1 ... H-1]. May contain multiple
// entries for the same value (pushed each time its frequency transitions
// to f) and stale entries (frequency has since moved on); both are
// discarded lazily / on lookup rather than eagerly removed.
...
void process_query(const Query& qry) {
    ...
    for (int f = start_both; f <= end_both; f++) {
        // find v1 = best (max orig_val) value currently at frequency f
        while (!max_heap[f].empty() && freq[max_heap[f].top()] != f) {
            max_heap[f].pop();
        }
        if (max_heap[f].empty()) continue;
        int v1 = max_heap[f].top();
        max_heap[f].pop();

        // find v2 = best DISTINCT value currently at frequency f.
        // Entries equal to v1 are genuine duplicates (pushed on an earlier
        // add/remove that also landed on frequency f) - they are permanently
        // discarded here since v1 already represents that value; this keeps
        // the total pop count amortized against the total push count
        // (one push per add/remove call), not against duplicate regrowth.
        int v2 = 0;
        bool found_v2 = false;
        while (!max_heap[f].empty()) {
            int cand = max_heap[f].top();
            if (freq[cand] != f) {
                max_heap[f].pop(); // stale, discard
                continue;
            }
            if (cand == v1) {
                max_heap[f].pop(); // genuine duplicate of v1, discard
                continue;
            }
            v2 = cand; // distinct and valid - leave it in the heap
            found_v2 = true;
            break;
        }
        ...
        max_heap[f].push(v1); // restore v1, which was popped above
    }
...
int32_t main() {
    ...
    for (int i = 0; i < q_num; i++) {
        ...
        if (type == 1) {
            int pos, val;
            cin >> pos >> val;
            updates.push_back({pos, temp_a[pos - 1], val});
            temp_a[pos - 1] = val;
    ...
    // Populate orig_val for EVERY compressed id, not just ones from the
    // initial array - values introduced solely via update new_vals also
    // need their orig_val set, otherwise they default to 0.
    for (int id = 1; id <= (int)all_vals.size(); id++) {
